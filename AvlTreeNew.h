...
protected:
    struct Node{
        ...
        Node(type data, const key& Id):data(data),Id(Id), right(NULL), left(NULL), height(0) {}
    ...
    virtual  const key& getKey(Node * cur)const {
        return cur->getId();
    }
...
public:
    ...
    virtual void deleteNode(Node * cur){}
...
public:
    ...
    virtual void add(type newData, const key& Id) {
        try {
            Node* newNode = new Node(newData, Id);
